# -*- coding: utf-8 -*-
"""
Created on Thu Aug  8 17:20:08 2019
...
def get_pictureandvideo(ID,x,page):
    ...
    for item in cards:
        ...
        if pics != '0':
            ...
            if 'status/' not in title_picture:
                #print('not in\n')
                title_picture = BeautifulSoup(title_picture,'lxml').get_text().strip().replace('.','').replace('\n','')
                title_picture = re.sub(r'[<|>|\|/|:|"|*|? ]','',title_picture)
                print(title_picture)
            if 'status/' in title_picture:
                #print('in\n')
                pat1 = '<a href="(.*?)">全文'
                title_data = re.compile(pat1).findall(title_picture)[0]
                re_url = 'https://m.weibo.cn/statuses/extend?id='+title_data[8:]
                data = requests.get(re_url,headers = headers).json()
                #print(data['data']['longTextContent'])
                data = data['data']['longTextContent']
                pat = ''
                title_picture = re.sub(r'<.*?>',' ',data)
                print(title_picture)
            if len(title_picture) >15:
                Incomplete_title_picture = title_picture[:15]
                
            else:
                Incomplete_title_picture = title_picture
            
            if not os.path.exists('./{}/picture/{}'.format(ID,Incomplete_title_picture)):
                    os.mkdir('./{}/picture/{}'.format(ID,Incomplete_title_picture))
            with open(r'./{}/picture/{}/{}.txt'.format(ID,Incomplete_title_picture,'标题'),'w') as f:
                f.write(title_picture)
            ik = 0
            for item in pics:
                ...
                ik += 1
                with open(r'./{0}/picture/{1}/{2}.jpg'.format(ID,Incomplete_title_picture,ik),'wb') as f:
                    f.write(r.content)
